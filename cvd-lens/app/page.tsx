...
const FEATURES = [
  ...
  },
  {
    ...
    detail: "원본과 보정본을 슬라이더로 비교 · Protan / Deutan / Tritan 3종 지원 · 브라우저 내부 추론.",
  ...
  },
  {
    ...
    href: "/correction",
    title: "실시간 카메라 보정",
    subtitle: "디바이스 카메라 영상을 실시간으로 분석해 색을 즉시 보정합니다.",
    detail: "MobileNetV2 U-Net · 24fps WebGL 가속 · 강도 슬라이더로 보정 정도 조절 가능.",
    tags: ["Live", "24 fps", "WebGL"],
  },
];
...
export default function Home() {
  return (
    <div className="max-w-[1280px] mx-auto px-6 md:px-10">
      ...
      <section className="grid md:grid-cols-[1.1fr_1fr] gap-12 md:gap-16 items-center py-16 md:py-20">
        <div>
          ...
          <p className="text-[17px] leading-relaxed mb-8 max-w-[480px]" style={{ color: "var(--fg-muted)" }}>
            색각이상(Color Vision Deficiency)을 가진 사용자를 위한 실시간 보정 도구.
            자가 진단부터 사진·실시간 카메라 보정까지, 한 곳에서 해결하세요.
          </p>
      ...
      {/* Footer strip */}
      <section className="grid grid-cols-2 md:grid-cols-4 gap-4 my-12">
        {[
          { k: "MODEL", v: "MobileNetV2 U-Net" },
          { k: "RUNTIME", v: "ONNX / WASM" },
          { k: "SUPPORTED", v: "Protan · Deutan · Tritan" },
          { k: "LATENCY", v: "≈ 42 ms / frame" },
        ].map((c) => (
          <div
            key={c.k}
            ...
            style={{ background: "var(--bg-elevated)", borderColor: "var(--border)" }}
          >
            <div className="font-mono text-[11px] tracking-[0.12em] uppercase mb-2" style={{ color: "var(--fg-subtle)" }}>
              {c.k}
            </div>
            <div className="text-[18px] font-medium tracking-[-0.01em]">{c.v}</div>
          </div>
        ))}
      </section>
    </div>
