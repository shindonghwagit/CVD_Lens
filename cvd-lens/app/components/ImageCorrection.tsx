...
export default function ImageCorrection() {
  const { ready, error, infer, simulate, preload, backend, lastMs } = useModel();
  ...
  // Lazy-load the model when THIS surface mounts (not the landing page).
  useEffect(() => { preload(cvdType); }, [preload, cvdType]);

  const computeSims = useCallback(async (type: CVDType) => {
    if (!sourceIDRef.current || !correctedIDRef.current) return;
    const [so, sc] = await Promise.all([
      simulate(sourceIDRef.current, type),
      simulate(correctedIDRef.current, type),
    ]);
    setSimOrig(imageDataToURL(so));
    setSimOut(imageDataToURL(sc));
  }, [simulate]);
  ...
  const runInference = useCallback(async (type: CVDType, sev: number) => {
    ...
    try {
      ...
      if (showSim) await computeSims(type);
    } finally {
      setProcessing(false);
      setSaveState("idle");
    }
  ...
  // Debounced re-infer on severity change (~150ms).
  ...
  const onSeverity = useCallback((v: number) => {
    ...
    sevTimer.current = setTimeout(() => runInference(cvdType, v), 150);
